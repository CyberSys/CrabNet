...
Description: Tests / Demonstrates how to use the RPC3 plugin. It is used for remote procedure calls where the call format is very similar to a local function call. Features over prior system (AutoRPC) include automatic NetworkIDObject object lookup, support for inheritance from NetworkIDObject anywhere, easier function registration, and support for custom object serialization via operator << and >> with CrabNet's Bitstream class as a target
