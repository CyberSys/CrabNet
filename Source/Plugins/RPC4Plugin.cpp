...
{
    void ( *registerFunctionPointer ) ( CrabNet::BitStream *userData, Packet *packet );
    void ( *registerBlockingFunctionPointer ) ( CrabNet::BitStream *userData, CrabNet::BitStream *returnData, Packet *packet );
...
RPC4GlobalRegistration::RPC4GlobalRegistration(const char* uniqueID, void ( *functionPointer ) ( CrabNet::BitStream *userData, Packet *packet ))
{
...
RPC4GlobalRegistration::RPC4GlobalRegistration(const char* uniqueID, void ( *functionPointer ) ( CrabNet::BitStream *userData, Packet *packet ), int callPriority)
{
    RakAssert(globalRegistrationIndex!=RPC4_GLOBAL_REGISTRATION_MAX_FUNCTIONS);
    unsigned int i;
    for (i=0; uniqueID[i]; i++)
...
RPC4GlobalRegistration::RPC4GlobalRegistration(const char* uniqueID, void ( *functionPointer ) ( CrabNet::BitStream *userData, CrabNet::BitStream *returnData, Packet *packet ))
{
    RakAssert(globalRegistrationIndex!=RPC4_GLOBAL_REGISTRATION_MAX_FUNCTIONS);
    unsigned int i;
    for (i=0; uniqueID[i]; i++)
...
bool RPC4::RegisterFunction(const char* uniqueID, void ( *functionPointer ) ( CrabNet::BitStream *userData, Packet *packet ))
{
...
void RPC4::RegisterSlot(const char *sharedIdentifier, void ( *functionPointer ) ( CrabNet::BitStream *userData, Packet *packet ), int callPriority)
{
...
bool RPC4::RegisterBlockingFunction(const char* uniqueID, void ( *functionPointer ) ( CrabNet::BitStream *userData, CrabNet::BitStream *returnData, Packet *packet ))
{
...
void RPC4::CallLoopback( const char* uniqueID, CrabNet::BitStream * bitStream )
{
    Packet *p=0;
...
void RPC4::Call( const char* uniqueID, CrabNet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast )
{
    CrabNet::BitStream out;
    out.Write((MessageID) ID_RPC_PLUGIN);
    out.Write((MessageID) ID_RPC4_CALL);
...
bool RPC4::CallBlocking( const char* uniqueID, CrabNet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, CrabNet::BitStream *returnData )
{
    CrabNet::BitStream out;
    out.Write((MessageID) ID_RPC_PLUGIN);
    out.Write((MessageID) ID_RPC4_CALL);
...
void RPC4::Signal(const char *sharedIdentifier, CrabNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, bool invokeLocal)
{
    CrabNet::BitStream out;
    out.Write((MessageID) ID_RPC_PLUGIN);
    ...
    if (invokeLocal)
    {
        ...
        CrabNet::BitStream *bsptr, bstemp;
        if (bitStream)
        {
            bitStream->ResetReadPointer();
...
void RPC4::InvokeSignal(DataStructures::HashIndex functionIndex, CrabNet::BitStream *serializedParameters, Packet *packet)
{
    if (functionIndex.IsInvalid())
        return;
    ...
    if (packet->data[0]==ID_RPC_PLUGIN)
    {
        ...
        if (packet->data[1]==ID_RPC4_CALL)
        {
            ...
            if (isBlocking==false)
            {
                ...
                void ( *fp ) ( CrabNet::BitStream *, Packet * );
            ...
            else
            {
                ...
                void ( *fp ) ( CrabNet::BitStream *, CrabNet::BitStream *, Packet * );
                fp = registeredBlockingFunctions.ItemAtIndex(skhi);
                CrabNet::BitStream returnData;
                bsIn.AlignReadToByteBoundary();
        ...
        {
            CrabNet::RakString sharedIdentifier;
            bsIn.ReadCompressed(sharedIdentifier);
            DataStructures::HashIndex functionIndex;
            functionIndex = localSlots.GetIndexOf(sharedIdentifier);
            CrabNet::BitStream serializedParameters;
            bsIn.AlignReadToByteBoundary();
