...
public:
    ...
    // Return -1 to use CrabNet's normal recvfrom, 0 to abort CrabNet's normal recvfrom, and positive to return data
    virtual int RakNetRecvFrom( char dataOut[ MAXIMUM_MTU_SIZE ], SystemAddress *senderOut, bool calledFromMainThread )=0;
};
*/

// A platform independent implementation of Berkeley sockets, with settings used by CrabNet
...
//    static void RecvFromBlocking_IPV4( RakNetSocket *s, RakPeer *rakPeer, char *dataOut, int *bytesReadOut, SystemAddress *systemAddressOut, CrabNet::TimeUS *timeRead );
//     #if CRABNET_SUPPORT_IPV6==1
//         static void RecvFromBlockingIPV4And6( RakNetSocket *s, RakPeer *rakPeer, char *dataOut, int *bytesReadOut, SystemAddress *systemAddressOut, CrabNet::TimeUS *timeRead );
//     #endif
//    static void RecvFromBlocking( RakNetSocket *s, RakPeer *rakPeer, char *dataOut, int *bytesReadOut, SystemAddress *systemAddressOut, CrabNet::TimeUS *timeRead );
    ...
    static CrabNet::RakString GetSubNetForSocketAndIp(__UDPSOCKET__ inSock, CrabNet::RakString inIpString);
