...
// TODO - CrabNet 4 - Add network simulator
// TODO - CrabNet 4 - Enable disabling flow control per connections
...
public:
    ...
    ConnectionAttemptResult Connect(const char* host, unsigned short remotePort, const char *passwordData, int passwordDataLength, PublicKey *publicKey=0, unsigned connectionSocketIndex=0, unsigned sendConnectionAttemptCount=6, unsigned timeBetweenSendConnectionAttemptsMS=1000, CrabNet::TimeMS timeoutTime=0 ) final;
    ...
    /// \return CONNECTION_ATTEMPT_STARTED on successful initiation. Otherwise, an appropriate enumeration indicating failure.
    /// \note CONNECTION_ATTEMPT_STARTED does not mean you are already connected!
    virtual ConnectionAttemptResult ConnectWithSocket(const char* host, unsigned short remotePort, const char *passwordData, int passwordDataLength, RakNetSocket2* socket, PublicKey *publicKey=0, unsigned sendConnectionAttemptCount=6, unsigned timeBetweenSendConnectionAttemptsMS=1000, CrabNet::TimeMS timeoutTime=0);
    ...
    uint32_t Send( const CrabNet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, uint32_t forceReceiptNumber=0 );
    ...
    void AddToBanList( const char *IP, CrabNet::TimeMS milliseconds=0 );
    ...
    /// \param[in] systemIdentifier Which system we are referring to
    CrabNet::Time GetClockDifferential( const AddressOrGUID systemIdentifier );
    ...
    void SetTimeoutTime( CrabNet::TimeMS timeMS, const SystemAddress target );
    ...
    CrabNet::TimeMS GetTimeoutTime( const SystemAddress target );
    ...
    /// \internal
    virtual void WriteOutOfBandHeader(CrabNet::BitStream *bitStream);
    ...
    struct RemoteSystemStruct
    {
        ...
        CrabNet::Time pingAndClockDifferentialWriteIndex;  /// The index we are writing into the pingAndClockDifferential circular buffer
        unsigned short lowestPing; ///The lowest ping value encountered
        CrabNet::Time nextPingTime;  /// When to next ping this player
        CrabNet::Time lastReliableSend; /// When did the last reliable send occur.  Reliable sends must occur at least once every timeoutTime/2 units to notice disconnects
        CrabNet::Time connectionTime; /// connection time, if active.
//        int connectionSocketIndex; // index into connectionSockets to send back on.
    ...
    // DS_APR
    //void ProcessChromePacket(RakNetSocket2 *s, const char *buffer, int dataSize, const SystemAddress& recvFromAddress, CrabNet::TimeUS timeRead);
    // /DS_APR
protected:
    ...
    friend bool ProcessOfflineNetworkPacket( SystemAddress systemAddress, const char *data, unsigned int length, RakPeer *rakPeer, RakNetSocket2* rakNetSocket, bool *isOfflineMessage, CrabNet::TimeUS timeRead );
    friend void ProcessNetworkPacket( const SystemAddress systemAddress, const char *data, unsigned int length, RakPeer *rakPeer, CrabNet::TimeUS timeRead, BitStream &updateBitStream );
    friend void ProcessNetworkPacket( const SystemAddress systemAddress, const char *data, unsigned int length, RakPeer *rakPeer, RakNetSocket2* rakNetSocket, CrabNet::TimeUS timeRead, BitStream &updateBitStream );
    ...
    void OnConnectionRequest( RakPeer::RemoteSystemStruct *remoteSystem, CrabNet::Time incomingTimestamp );
    ...
    // CrabNet::LocklessUint32_t isRecvFromLoopThreadActive;
    ...
    CrabNet::BitStream offlinePingResponse;
    ///Local Player ID
...
//    unsigned int FirstFreeRemoteSystemLookupIndex(const SystemAddress &sa) const;
    ...
    {
        char *IP;
        CrabNet::TimeMS timeout; // 0 for none
    };

    struct RequestedConnectionStruct
    {
        SystemAddress systemAddress;
        CrabNet::Time nextRequestTime;
...
//        char remote_challenge[cat::EasyHandshake::CHALLENGE_BYTES];
    ...
    CrabNet::SimpleMutex bufferedPacketsFreePoolMutex;
    DataStructures::Queue<RNS2RecvStruct*> bufferedPacketsQueue;
    CrabNet::SimpleMutex bufferedPacketsQueueMutex;
    ...
    CrabNet::TimeMS unreliableTimeout;
    ...
    DataStructures::List<CrabNet::RakString> securityExceptionList;
