...
protected:
    ...
    CrabNet::Time whenJoinAnyRequested;
...
protected:
    ...
    void BroadcastToParticipants(CrabNet::BitStream *bsOut, RakNetGUID exclusionGuid);
...
protected:
    ...
    virtual void OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, PI2_LostConnectionReason lostConnectionReason );
    virtual void OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, bool isIncoming);
    void Send( const CrabNet::BitStream * bitStream, const AddressOrGUID systemIdentifier, bool broadcast );

    void EncodeTeamFullOrLocked(CrabNet::BitStream *bitStream, TM_TeamMember *teamMember, TM_Team *team);
    void DecomposeTeamFullOrLocked(CrabNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember, TM_Team **team,
        uint16_t &currentMembers, uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive, JoinPermissions &joinPermissions);
    void ProcessTeamAssigned(CrabNet::BitStream *bsIn);

    void EncodeTeamAssigned(CrabNet::BitStream *bitStream, TM_TeamMember *teamMember);
    ...
    void PushBitStream(CrabNet::BitStream *bitStream);
    ...
    void EncodeTeamFull(CrabNet::BitStream *bitStream, TM_TeamMember *teamMember, TM_Team *team);
    void EncodeTeamLocked(CrabNet::BitStream *bitStream, TM_TeamMember *teamMember, TM_Team *team);

    /// \brief When you get ID_TEAM_BALANCER_TEAM_ASSIGNED, pass the packet to this function to read out parameters
    /// \param[in] A packet where packet->data[0]==ID_TEAM_BALANCER_TEAM_ASSIGNED
    /// \return true on success, false on read error
    void DecodeTeamAssigned(CrabNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember, NoTeamId &noTeamSubcategory,
