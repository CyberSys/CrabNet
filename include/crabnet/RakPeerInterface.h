...
public:
    ...
    /// \return CONNECTION_ATTEMPT_STARTED on successful initiation. Otherwise, an appropriate enumeration indicating failure.
    /// \note CONNECTION_ATTEMPT_STARTED does not mean you are already connected!
    virtual ConnectionAttemptResult ConnectWithSocket(const char* host, unsigned short remotePort, const char *passwordData, int passwordDataLength, RakNetSocket2* socket, PublicKey *publicKey=0, unsigned sendConnectionAttemptCount=12, unsigned timeBetweenSendConnectionAttemptsMS=500, CrabNet::TimeMS timeoutTime=0)=0;
    ...
    virtual uint32_t Send( const CrabNet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, uint32_t forceReceiptNumber=0 )=0;
    ...
    virtual void AddToBanList( const char *IP, CrabNet::TimeMS milliseconds=0 )=0;
    ...
    virtual CrabNet::Time GetClockDifferential( const AddressOrGUID systemIdentifier )=0;
    ...
    virtual void SetTimeoutTime( CrabNet::TimeMS timeMS, const SystemAddress target )=0;
    ...
    virtual CrabNet::TimeMS GetTimeoutTime( const SystemAddress target )=0;
    ...
    virtual void SetUnreliableTimeout(CrabNet::TimeMS timeoutMS)=0;
    ...
    virtual void WriteOutOfBandHeader(CrabNet::BitStream *bitStream)=0;
