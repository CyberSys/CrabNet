...
/// \brief Encapsulates UDT congestion control, as used by CrabNet
/// Requirements:
/// <OL>
...
class CCRakNetUDT
{
    ...
    void OnResend(CCTimeType curTime, CrabNet::TimeUS nextActionTime);
    ...
    /// This is per message, which is different from UDT, but CrabNet supports packetloss with continuing data where UDT is only RELIABLE_ORDERED
...
//    void SetTimeBetweenSendsLimit(unsigned int bitsPerSecond);
    ...
    /// Used to calculate packet arrival rate (in UDT) but data arrival rate (in CrabNet, where not all datagrams are the same size)
