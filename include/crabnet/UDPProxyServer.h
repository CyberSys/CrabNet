...
struct UDPProxyServerResultHandler
{
    ...
    virtual void OnLoginSuccess(CrabNet::RakString usedPassword, CrabNet::UDPProxyServer *proxyServerPlugin)=0;

    /// We are already logged in.
    /// This login failed, but the system is operational as if it succeeded
    /// \param[out] usedPassword The password we passed to UDPProxyServer::LoginToCoordinator()
    /// \param[out] proxyServer The plugin calling this callback
    virtual void OnAlreadyLoggedIn(CrabNet::RakString usedPassword, CrabNet::UDPProxyServer *proxyServerPlugin)=0;

    /// The coordinator operator forgot to call UDPProxyCoordinator::SetRemoteLoginPassword()
    /// \param[out] usedPassword The password we passed to UDPProxyServer::LoginToCoordinator()
    /// \param[out] proxyServer The plugin calling this callback
    virtual void OnNoPasswordSet(CrabNet::RakString usedPassword, CrabNet::UDPProxyServer *proxyServerPlugin)=0;

    /// The coordinator operator set a different password in UDPProxyCoordinator::SetRemoteLoginPassword() than what we passed
    /// \param[out] usedPassword The password we passed to UDPProxyServer::LoginToCoordinator()
    /// \param[out] proxyServer The plugin calling this callback
    virtual void OnWrongPassword(CrabNet::RakString usedPassword, CrabNet::UDPProxyServer *proxyServerPlugin)=0;
};
...
public:
    ...
    bool LoginToCoordinator(CrabNet::RakString password, SystemAddress coordinatorAddress);
