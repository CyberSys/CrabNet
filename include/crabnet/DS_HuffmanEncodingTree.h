...
public:
    ...
    void EncodeArray(unsigned char *input, size_t sizeInBytes, CrabNet::BitStream * output);

    // \brief Decodes an array encoded by EncodeArray().
    unsigned DecodeArray(CrabNet::BitStream * input, BitSize_t sizeInBits, size_t maxCharsToWrite, unsigned char *output);
    void DecodeArray(unsigned char *input, BitSize_t sizeInBits, CrabNet::BitStream * output);
