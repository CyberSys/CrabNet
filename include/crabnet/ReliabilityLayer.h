...
//    void Push2(CrabNet::TimeUS time, uint64_t value1, uint64_t value2);
...
//    uint64_t GetTotal2(void) const;
    ...
    //    TimeAndValue2(CrabNet::TimeUS t, uint64_t v1, uint64_t v2);
...
//    void ClearExpired2(CrabNet::TimeUS time);
};
...
public:
    ...
    void SetTimeoutTime( CrabNet::TimeMS time );
    ...
    //void GetUndeliveredMessages(CrabNet::BitStream *messages, int MTUSize);
...
private:
    ...
    void SendBitStream( RakNetSocket2 *s, SystemAddress &systemAddress, CrabNet::BitStream *bitStream, RakNetRandom *rnr, CCTimeType currentTime);

    ///Parse an internalPacket and create a bitstream to represent this data
    /// \return Returns number of bits used
    BitSize_t WriteToBitStreamFromInternalPacket( CrabNet::BitStream *bitStream, const InternalPacket *const internalPacket, CCTimeType curTime );


    /// Parse a bitstream and create an internal packet to represent this data
    InternalPacket* CreateInternalPacketFromBitStream( CrabNet::BitStream *bitStream, CCTimeType time );
...
//    void DeleteSequencedPacketsInList( unsigned char orderingChannel, DataStructures::Queue<InternalPacket*>&theList );
    ...
    //unsigned int IsReceivedPacketHole(unsigned int input, CrabNet::TimeMS currentTime) const;
    ...
    struct UnreliableWithAckReceiptNode
    {
        ...
        UnreliableWithAckReceiptNode(DatagramSequenceNumberType _datagramNumber, uint32_t _sendReceiptSerial, CrabNet::TimeUS _nextActionTime) :
...
//    CCTimeType timeResendQueueNonEmpty;
    CrabNet::TimeMS timeLastDatagramArrived;
...
//    double bytesInSendBuffer[NUMBER_OF_PRIORITIES];
    ...
    //CrabNet::BitStream updateBitStream;
    bool deadConnection, cheater;
    SplitPacketIdType splitPacketId;
    CrabNet::TimeMS timeoutTime; // How long to wait in MS before timing someone out
...
//    CCTimeType lastTimeBetweenPacketsIncrease,lastTimeBetweenPacketsDecrease;
    ...
    // Internet simulator
    double packetloss;
    CrabNet::TimeMS minExtraPing, extraPingVariance;
#endif
...
#if USE_SLIDING_WINDOW_CONGESTION_CONTROL==1
    CrabNet::CCRakNetSlidingWindow congestionManager;
#else
    CrabNet::CCRakNetUDT congestionManager;
#endif
